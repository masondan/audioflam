...
- ✅ **Ffion voice retired — enrollment-audio-leak bug, fixed by re-enrolling as "Carys"** — Ffion intermittently produced a recognizable garbled fragment of speech at the start (and occasionally end) of synthesis. Root cause confirmed via raw-WAV-dump diagnostics (bypassing all client-side processing): the leaked fragments were verbatim/partial matches of **Ffion's own ~20s enrollment reference audio** ("Explore the ice-carved slopes of the Brecon Beacons...") bleeding into unrelated synthesis output — a genuine Aliyun/DashScope-side defect in how the `qwen-audio-3.0-tts-flash` voice-enrollment pipeline handles reference-audio conditioning, not a bug in our WebSocket handling (frame ordering and delivery were confirmed complete and correctly sequenced). Original enrollment (`scripts/reclone_production_voices.js`) also omitted the documented `enable_preprocess` and `max_prompt_audio_length` parameters — all 6 production voices' reference clips were 1.9×–3.0× over the 10s `max_prompt_audio_length` default with `enable_preprocess` left at its `false` default. Fixed by re-enrolling a new voice ("Carys") with a short (~8s), clean reference clip and `enable_preprocess: true` — confirmed clean across 20+ test generations, including phrases deliberately echoing the new enrollment's own transcript. Carys now replaces Ffion everywhere in the UI (`QWEN_WELSH_VOICES` in `src/lib/stores.ts`); Ffion's old voice_id is retired and unused. Enrollment script: [`scripts/enroll_carys.js`](scripts/enroll_carys.js). Do not re-enroll any of the other 5 production voices preemptively — none have shown this symptom, though the same over-length/no-preprocess enrollment pattern applies to all of them and is worth keeping in mind if similar artifacts are ever reported.
- ✅ **Qwen WebSocket intermittent trailing-word/syllable truncation (~40% of short-text requests)** — Diagnostic logging proved Aliyun's `task-finished` text event does not reliably arrive after all binary audio frames; in a meaningful fraction of test runs, one or more trailing audio frames arrived on the WebSocket immediately after `task-finished` had already been processed and the response finalized, silently dropping the tail of the audio. (This is distinct from, and not caused by, the fixed 300ms `continue-task`→`finish-task` delay — sending `finish-task` early does not stop the server from continuing to generate/send audio, confirmed via complete-stream diagnostic captures.) Fixed by adding a 400ms grace period after `task-finished` before finalizing, plus awaiting the async frame-processing queue, so any straggler frames already in flight are captured. Waiting for the `close` event instead is not viable — observed delay between `task-finished` and `close` was ~128 seconds in testing. ([`+server.ts:476-495`](src/routes/api/tts/+server.ts:476))

**Known limitation (not yet investigated further):**
- 🟡 **Mercy: leading single-letter word truncation** — When text sent to Qwen begins with a standalone single letter (e.g. "A passion fruit..."), the letter is dropped/inaudible in Mercy's voice specifically. Confirmed via raw-WAV-dump diagnostic that this is present in Aliyun's own output, before any of our processing — not a bug in our code, not fixable by the trailing-truncation grace-period fix above, and a client-side text-merging workaround (hyphenating the leading letter into the next word before synthesis) did **not** resolve it. Deprioritized per user request (low near-term usage of Mercy). If revisited, consider testing whether the same enrollment recipe used for Carys (short clean clip + `enable_preprocess: true`) also resolves this, since Mercy's enrollment shares the same over-length/no-preprocess pattern as the original Ffion enrollment.
